...
def conv1x1(in_planes, out_planes, stride=1, PC=0):
    ...
    if PC:
        return Higham_norm.spectral_norm(nn.Conv2d(in_planes, out_planes, 
                kernel_size=1, stride=stride, bias=False),use_adaptivePC=False, pclevel=PC)
    else:
        return nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)
...
class FixupResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, PC=0):
        super(FixupResNet, self).__init__()
        self.num_layers = sum(layers)
        self.inplanes = 64
        if PC:
            self.conv1 = Higham_norm.spectral_norm(nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
