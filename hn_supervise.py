...
class SpectralNorm(object):
    ...
    def __init__(self, name='weight', n_power_iterations=1, dim=0, eps=1e-12, use_adaptivePC=False, pclevel=1, diter=1):
        self.name = name
        self.dim = dim
    ...
    def compute_weight(self, module, do_power_iteration):
        ...
        # sigma = S[0]
        weight = weight / sigma0
        if self.called_time % 250 == 0:
            # print("sigma", sigma)
            # S = torch.svd(weight_mat)[1]
            print("estimate sigma", sigma0)
            # print("real sigma", S[0])
            # print("sigma difference", (sigma0-S[0]).abs())
            # print("before precondition", S[0]/S[-1])
        # # --------- remove later -------------
        # if self.called_time % 250 == 0:
    ...
    def preconditionerwide(self, weight, pclevel):
        ...
        if self.called_time % 250 == 0:
            ...
            print("after precondition condinumb", S[0]/S[-1])
        return weight
    ...
    def remove(self, module):
        ...
        delattr(module, self.name + '_pcleval')
        delattr(module, self.name + '_cns')
    ...
    @staticmethod
    def apply(module, name, n_power_iterations, dim, eps, use_adaptivePC, pclevel, diter):
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, SpectralNorm) and hook.name == name:
                raise RuntimeError("Cannot register two spectral_norm hooks on "
                                   "the same parameter {}".format(name))

        fn = SpectralNorm(name, n_power_iterations, dim, eps, use_adaptivePC, pclevel, diter)
        ...
        #         module.register_parameter(fn.name + "_snorm", weight)
        #         module.register_parameter(fn.name + "_higham", weight)
...
def spectral_norm(module, name='weight', n_power_iterations=1, eps=1e-12, dim=None, use_adaptivePC=True, pclevel=0, diter=1):
    ...
    if dim is None:
        if isinstance(module, (torch.nn.ConvTranspose1d,
            ...
            dim = 0
    SpectralNorm.apply(module, name, n_power_iterations, dim, eps, use_adaptivePC=use_adaptivePC, pclevel=pclevel, diter=diter)
    return module
